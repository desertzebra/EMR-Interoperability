...
class AnnotatedDataHandler:
    ...
    def get_kappa_correlation_score(self, list1, list2):

        kappaCorrelationBetweenLists = []
        list1Class = [t[2] for t in list1]
        list2Class = [t[2] for t in list2]

        print('list1Class: ', len(list1Class), ' list2Class: ', len(list2Class))
    ...
    def calculateKappaScoreBetweenAnnotators(self):
        ...
        annotator1DataFlat = self.collapseDataSetTo1dArrayWithHeaders(self.annotator1Data)
        annotator2DataFlat = self.collapseDataSetTo1dArrayWithHeaders(self.annotator2Data)
        annotator3DataFlat = self.collapseDataSetTo1dArrayWithHeaders(self.annotator3Data)
        annotator4DataFlat = self.collapseDataSetTo1dArrayWithHeaders(self.annotator4Data)
        ...
        allKappaAnnotatedDataHandlers += "annotator1 vs annotator4," + \
                                         str(annotatedDataHandler.get_kappa_correlation_score(annotator1DataFlat,
                                                                                          annotator4DataFlat)) + "\r\n"
        self.log(["*"] * 80)

        self.log('Cohen kappa score  between annotator2 and annotator3: ')
        allKappaAnnotatedDataHandlers += "annotator2 vs annotator3," + \
                                         str(annotatedDataHandler.get_kappa_correlation_score(annotator2DataFlat,
                                                                                          annotator3DataFlat)) + "\r\n"
        self.log(["*"] * 80)
        ...
        allKappaAnnotatedDataHandlers += "annotator3 vs annotator4," + \
                                         str(annotatedDataHandler.get_kappa_correlation_score(annotator3DataFlat,
                                                                                          annotator4DataFlat)) + "\r\n"
        annotatedDataHandler.log(["*"] * 80)
...
print('print dataset detail')

print('dataset["test_y"]: ', len(dataset['test_y']))
print('dataset["dev_y"]: ', len(dataset['dev_y']))
print('flatAnnotatedData', len(flatAnnotatedData))



annotatedDataHandler.log(['Development: Class0=%d, Class1=%d, Class2=%d' % (
            len([t[2] for t in dataset['dev_y'] if t[2] == annotatedDataHandler.class_unrelated]),
            len([t[2] for t in dataset['dev_y'] if t[2] == annotatedDataHandler.class_related]),
            len([t[2] for t in dataset['dev_y'] if t[2] == annotatedDataHandler.class_equal]))])


annotatedDataHandler.log(['Test: Class0=%d, Class1=%d, Class2=%d' % (
            len([t[2] for t in dataset['test_y'] if t[2] == annotatedDataHandler.class_unrelated]),
            len([t[2] for t in dataset['test_y'] if t[2] == annotatedDataHandler.class_related]),
            len([t[2] for t in dataset['test_y'] if t[2] == annotatedDataHandler.class_equal]))])

annotatedDataHandler.read_all_computed_data(read_headers=True)
