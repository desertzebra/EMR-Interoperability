...
\begin{algorithm}
	\textbf{Input}: $ AttributeRepresentation AR_1, AR_2$ \\
	\textbf{Output}: Similarity $S$ 
	\begin{algorithmic}[1]
		\State Similarity $S = 0$;
		\If{$AR_1.AttributeContext == AR_2.AttributeContext$}:
			\State return 1;
		\ElsIf{$AR_1.AttributeType == AR_2.AttributeType$}:
			\State	$S = 0.2$
		\ElsIf{$AR_1.AttributeType <--> AR_2.AttributeType$}:
			\State $S = 0.2$
		\EndIf
		\If{$AR_1.AttributSemantics$}:
			\State $S += \frac{AR_1.AttributeSemantics.TreeEmbedding \cdot AR_2.AttributeSemantics.TreeEmbedding}{\|A\|\|B\|}$
		\EndIf
